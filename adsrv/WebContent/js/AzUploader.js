/*
 * AzUploader 1.0.0 2010.09.25
 * Copyright(c) 20010-2010 DEVARZZ.
 * arzz@arzz.com
 * http://dev.arzz.com/AzUploader/license.arzz
 */
	
if(isIncludeAzUploader===undefined){
	var isIncludeAzUploader=true;
var AzUploaderComponent={
	components:new Array(),get:function(id){
	...
	this.get(id).readyQueue.push("AzUploaderComponent.get(\""+id+"\").setLoadURLParams(\""+params+"\");
 AzUploaderComponent.get(\""+id+"\").load();
");
}else{
	this.get(id).setLoadURLParams(params);
this.get(id).load();
}}},error:function(code){
	switch(code){
	case 101:this.errormsg("버튼이미지를 로딩하지 못하였습니다.");
break;
case 201:this.errormsg("업로드할 파일이 없습니다.");
break;
case 202:this.errormsg("업로드경로가 잘못지정되었거나, 해당경로에 파일이 없습니다.");
break;
case 203:this.errormsg("업로드경로는 업로더가 올려진 도메인내의 경로만 가능합니다.");
break;
case 999:this.errormsg("AzUploader가 로딩되지 않았습니다.");
break;
...
switch(type){
	case"onReady":AzUploader.flash=document.getElementById(id);
AzUploader.isReady=true;
for(var i=0,loop=AzUploader.readyQueue.length;
i<loop;
i++){
...
try{
	AzUploader.listeners.onReady(AzUploader);
}catch(e){
	}
break;
case"onSelect":var fileList=new Array();
for(var i=0,loop=data.length;
i<loop;
i++){
	fileList[i]={
	};
...
try{
	AzUploader.listeners.onSelect(AzUploader,fileList);
}catch(e){
	}
break;
case"onError":file={
...
try{
	AzUploader.listeners.onError(AzUploader,data[3],file);
}catch(e){
	}
break;
case"onDelete":file={
...
file.size=data[2];
try{
	AzUploader.listeners.onDelete(AzUploader,file);
}catch(e){
	}
break;
case"onUpload":var file={
	};
file.idx=data[0];
file.name=data[1];
file.size=data[2];
file.server=data[3];
try{
	AzUploader.listeners.onUpload(AzUploader,file);
}catch(e){
	}
break;
case"onLoad":var file={
	};
file.idx=data[0];
file.name=data[1];
file.size=data[2];
file.server=data[3];
try{
	AzUploader.listeners.onLoad(AzUploader,file);
}catch(e){
	}
break;
case"onProgress":var file={
	};
file.idx=data[0];
file.name=data[1];
file.size=data[2];
...
try{
	AzUploader.listeners.onProgress(AzUploader,file,fileUpload,totalUpload,time,speed);
}catch(e){
	}
break;
case"onComplete":try{
	AzUploader.listeners.onComplete(AzUploader,file);
}catch(e){
	}
...
this.AzUploaderHTML+='<embed src="'+this.flashURL+'" quality="high" wmode="transparent" style="width:'+this.width+'px;
 height:'+this.height+'px;
" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" flashvars="'+this.flashVars+'"></embed>';
...
");
	...
	this.AzUploaderHTML='<embed id="'+this.id+'" src="'+this.flashURL+'" quality="high" wmode="transparent" style="width:'+this.width+'px;
 height:'+this.height+'px;
" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" base="." flashvars="'+this.flashVars+'"></embed>';
if(this.autoRender==true){
	if(this.renderElement)document.getElementById(this.renderElement).innerHTML=this.AzUploaderHTML;
else document.write(this.AzUploaderHTML);
}}
this.render=function(renderElement){
	if(renderElement)document.getElementById(renderElement).innerHTML=this.AzUploaderHTML;
else document.write(this.AzUploaderHTML);
...
this.remove=function(idx){
	if(this.isReady==false)return;
this.flash.remove(idx);
}
this.load=function(){
	if(this.isReady==false)return;
if(this.listeners.beforeLoad&&this.listeners.beforeLoad(this)==false)return;
this.flash.load();
}
this.setLoadURLParams=function(params){
	if(this.isReady==false)return;
this.loadURL+=this.loadURL.indexOf("?")==-1?"?"+params:"&"+params;
this.flash.setLoadURL(this.loadURL);
}
this.setAllowType=function(allowType){
	if(this.isReady==false)return;
this.flash.setAllowType(allowType);
}
this.setMaxFileSize=function(maxFileSize){
	if(this.isReady==false)return;
this.flash.setMaxFileSize(maxFileSize);
}
this.setMaxTotalSize=function(maxTotalSize){
	if(this.isReady==false)return;
this.flash.setMaxTotalSize(maxTotalSize);
}
